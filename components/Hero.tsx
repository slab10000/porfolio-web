...
const Hero: React.FC = () => {
	...
	const [glassPosition2, setGlassPosition2] = useState({
		top: 0,
		left: 0,
		width: 0,
		height: 0,
	});
	const [glassPosition3, setGlassPosition3] = useState({
		top: 0,
		left: 0,
		width: 0,
		height: 0,
	...
	useEffect(() => {
		const updatePosition = () => {
			...
			const orbYInImage = imgNaturalHeight * 0.4; // 15% from top (moved down)

			// Convert to container coordinates
			const orbXInContainer =
				(orbXInImage / imgNaturalWidth) * displayedWidth + offsetX;
			const orbYInContainer =
				(orbYInImage / imgNaturalHeight) * displayedHeight + offsetY;
			...
			const orb2XInContainer =
				(orb2XInImage / imgNaturalWidth) * displayedWidth + offsetX;
			const orb2YInContainer =
				(orb2YInImage / imgNaturalHeight) * displayedHeight + offsetY;
			...
			const orb3XInImage = imgNaturalWidth * 0.3; // 15% from left (left part)
			const orb3YInImage = imgNaturalHeight * 0.2; // Moved up more

			const orb3XInContainer =
				(orb3XInImage / imgNaturalWidth) * displayedWidth + offsetX;
			const orb3YInContainer =
				(orb3YInImage / imgNaturalHeight) * displayedHeight + offsetY;
	...
	return (
		<section
			id="home"
			className="relative pt-32 pb-40 overflow-hidden min-h-screen flex flex-col justify-center"
		>
					...
					<div className="relative w-full h-full">
						...
						<div className="absolute inset-4 bg-white/30 backdrop-blur-xl rounded-full border border-white/50 shadow-2xl flex items-center justify-center overflow-hidden">
							<div
								...
								style={{
									background:
										"radial-gradient(ellipse at 25% 50%, rgba(211, 48, 184, 0.7) 0%, rgba(211, 48, 184, 0.5) 40%, rgba(233, 189, 63, 0.6) 60%, rgba(245, 200, 66, 0.8) 80%, rgba(255, 215, 0, 0.9) 100%)",
								}}
							></div>
						</div>
					</div>
					...
					<div className="relative w-full h-full">
						...
						<div className="absolute inset-4 bg-white/30 backdrop-blur-xl rounded-full border border-white/50 shadow-2xl flex items-center justify-center overflow-hidden">
							<div
								...
								style={{
									background:
										"radial-gradient(ellipse at 25% 50%, rgba(51, 220, 207, 0.7) 0%, rgba(51, 220, 207, 0.5) 40%, rgba(74, 229, 216, 0.6) 60%, rgba(95, 236, 224, 0.8) 80%, rgba(111, 255, 245, 0.9) 100%)",
								}}
							></div>
						</div>
					</div>
					...
					<div className="relative w-full h-full">
						...
						<div className="absolute inset-4 bg-white/30 backdrop-blur-xl rounded-full border border-white/50 shadow-2xl flex items-center justify-center overflow-hidden">
							<div
								...
								style={{
									background:
										"radial-gradient(ellipse at 25% 50%, rgba(109, 12, 255, 0.7) 0%, rgba(109, 12, 255, 0.5) 15%, rgba(138, 47, 255, 0.6) 28%, rgba(184, 74, 255, 0.8) 40%, rgba(236, 7, 245, 0.95) 55%, rgba(255, 10, 255, 1) 80%, rgba(255, 0, 255, 1) 100%)",
								}}
							></div>
						</div>
					</div>
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 w-full">
				...
				<div className="flex items-center gap-4 mb-8">
					...
					<div className="text-xs font-mono text-white">EST. 2025</div>
				</div>
				...
				<div className="relative">
					...
					<a
						...
						className="inline-flex items-center gap-3 px-5 py-3 rounded-full bg-white/20 backdrop-blur-md border border-white/30 text-white font-medium text-sm sm:text-base hover:bg-white/30 hover:border-white/50 transition-colors"
					>
						<span className="px-2 py-1 rounded-full bg-swiss-lime text-black text-xs font-black uppercase tracking-[0.2em]">
							New
						</span>
						<span>
							🏆 Won the San Francisco edition of Mistral AI&apos;s Worldwide
							Hackathon 🏆
						</span>
					</a>
				</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-12 gap-8 mt-48 items-end">
					<div className="md:col-span-7">
						<p className="text-2xl md:text-3xl font-light leading-tight text-white max-w-2xl">
							An{" "}
							<span
								className="font-medium px-1 text-black"
								style={{ backgroundColor: "#d6fe51" }}
							>
								AI Engineer
							</span>{" "}
							and roboticist orchestrating the convergence of digital
							intelligence and physical systems.
						</p>
					</div>
					<div className="md:col-span-5 flex flex-col items-start md:items-end mt-12 md:mt-0">
						<div className="flex gap-4">
							<a
								href="#projects"
								className="group flex items-center gap-2 px-8 py-8 bg-swiss-black text-white rounded-full hover:bg-swiss-blue transition-colors duration-300"
							>
					...
					<div className="flex animate-marquee whitespace-nowrap">
						...
						{[...Array(3)].map((_, i) => (
							<div key={`first-${i}`} className="flex items-center mx-8">
								<span className="text-4xl font-display font-bold text-transparent text-outline uppercase">
									AI Architecture
								</span>
								<Asterisk className="w-8 h-8 mx-6 text-swiss-lime" />
								<span className="text-4xl font-display font-bold text-swiss-black uppercase">
